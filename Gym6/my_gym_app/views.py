        ...
        # 2. Edit an existing user
            ...
            for existing_user in USERS_DATA:
                if existing_user['username'] == edit_username:
                    ...
                    # (so if a captain attempts to change an existing user to 'captin' or 'manager', it won't happen)
...
def captin_manage_users(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'add':
            # Captain can only add 'user'-type accounts
            new_username = request.POST.get('username')
            new_password = request.POST.get('password', '')
            new_type = request.POST.get('type', 'user')  # Force to 'user'

            # Ensure the captain doesn't create a manager or another captain
            if new_type != 'user':
                new_type = 'user'
